...
def load_equities_web(symbol, date_from):
    ...
    data = raw_data.stack(dropna=False)['Adj Close'].to_frame().reset_index().rename(
        columns={'Symbols': 'symbol', 'Date': 'date', 'Adj Close': 'value'}).sort_values(by=['symbol', 'date'])
    return pd.pivot_table(data, columns='symbol', index='date', values='value')


prices = load_equities_web(['SPY', '^GSPX', '^VIX'], date_from='2000-01-01')
...
def backtest_strategy(prices, symbol_trade, symbol_volatility, volatility_threshold, capital, symbol_benchmark):
    ...
    for date in calendar:
        ...
        if prices.loc[date, symbol_volatility] > volatility_threshold:  # volatility is high
    ...
    portval = (df_end * prices.assign(cash=1)[[symbol_trade, 'cash']]).sum(axis=1).to_frame().rename(columns=
                                                                                                   {0: 'strategy'})
...
res = backtest_strategy(prices=prices, symbol_trade='SPY',
                        symbol_volatility='^VIX', volatility_threshold=30, capital=10000,
